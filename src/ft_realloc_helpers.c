...
/*   Updated: 2020/01/27 13:47:50 by cpaquet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_malloc.h"

void		*ft_move_free_header(void *ptr, size_t chunk_size,
															size_t size_diff)
{
	size_t		flags;
	...
	*((t_chunk*)(free_block + size_diff)) = free_tmp;
	...
	set_prev_size(((t_chunk *)free_block)->mchunk_prevsize + size_diff,
	free_block + size_diff);
	if (chunk_size < TINY_MAX_SIZE)
		begin_free = &(g_ptr.tiny_free_begin);
	else
		begin_free = &(g_ptr.small_free_begin);
	*begin_free = (*begin_free == free_block ?
	free_block + size_diff : *begin_free);
...
void		ft_update_chunk_after_free(void *prev_ptr, size_t new_size,
								char prev_is_free, t_heapheader *current_heap)
{
	size_t size;
...
char		ft_is_same_heap_size(size_t size_chunk, size_t size_user)
{
	...
	&& (size_chunk >= TINY_MAX_SIZE && size_user >= TINY_MAX_SIZE))
		return (2);
	return (0);
}
