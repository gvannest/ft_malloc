...
/*   Updated: 2020/01/27 13:44:46 by cpaquet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		*ft_next_chunk_free(void *ptr, t_chunk *next_chunk,
			size_t size_user, t_heapheader *current_heap)
{
	size_t		size_diff;
	...
	if (next_size_wo_flags < size_diff + HDR_SIZE)
	{
		...
		ft_update_chunk_after_free(ptr, size_wo_flags
		+ next_size_wo_flags, 0, current_heap);
	}
	else
	{
		free_chunk = ft_move_free_header(ptr, size_wo_flags, size_diff);
		ft_update_current_size(ptr, size_wo_flags + size_diff);
		ft_update_chunk_after_free(free_chunk,
		next_size_wo_flags - size_diff, 1, current_heap);
		update_freelist(((t_chunk*)free_chunk)->prev_chunk,
		(t_chunk*)free_chunk, ((t_chunk*)free_chunk)->next_chunk);
	}
	return (ptr + HDR_SIZE);
}

void		*ft_reduce_chunk(void *ptr, size_t new_chunk_size,
											size_t actual_size, char heap_type)
{
	size_t		size_reduction;
	void		**begin_free;

	size_reduction = actual_size - new_chunk_size;
	begin_free = heap_type == 1 ?
	&(g_ptr.tiny_free_begin) : &(g_ptr.small_free_begin);
	if (new_chunk_size == actual_size || size_reduction < HDR_SIZE)
		return (ptr + HDR_SIZE);
	create_new_free_chunk(begin_free, ptr, new_chunk_size,
	find_current_heap(ptr));
	return (ptr + HDR_SIZE);
}
