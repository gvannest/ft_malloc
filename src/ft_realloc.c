...
static void *ft_move_free_header(void *ptr, size_t chunk_size, size_t size_diff)
{
	...
	free_tmp = *((t_freechunk*)free_block);
	*((t_freechunk*)(free_block + size_diff)) = free_tmp;
...
static void	*ft_reduce_chunk(void *ptr, size_t new_chunk_size, size_t actual_size, char heap_type)
{
	...
	if (new_chunk_size == actual_size || size_reduction < HDR_SIZE_FREE)
		return (ptr + HDR_SIZE_ALLOC);
...
void *ft_realloc(void *ptr, size_t size)
{
	...
	size_aligned = (size_aligned < HDR_SIZE_FREE - HDR_SIZE_ALLOC ? HDR_SIZE_FREE - HDR_SIZE_ALLOC : size_aligned);
	...
	if (size_aligned + HDR_SIZE_ALLOC <= size_wo_flags)
