...
void			create_new_free_chunk(void **begin_free, t_chunk *selected_chunk, size_t chunk_size, t_heapheader *current_heap)
{
	t_chunk		*new_chunk;
	size_t			free_size;
	t_chunk		*prev;
	...
	{	
		*new_chunk = ft_set_header_free(chunk_size, ft_add_flags_to_size(free_size, 1, 0, 1), selected_chunk, selected_chunk->next_chunk);
	...
	if (next_block < current_heap->current_footer)
		*((t_chunk*)next_block) = ft_set_header_free(free_size, ((t_chunk*)next_block)->mchunk_size | P_FLAG, ((t_chunk*)next_block)->prev_chunk, ((t_chunk*)next_block)->next_chunk);
