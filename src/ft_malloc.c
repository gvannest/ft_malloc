...
/*   Updated: 2020/01/25 16:22:58 by gvannest         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		*ft_tiny_small(size_t size, size_t call_size, void **begin_free)
{
	...
	if (!(*begin_free))
	{
		if (!(*begin_free = call_mmap(call_size)))
			return (NULL);
		ft_first_free_chunk(*begin_free + HDR_HEAP,
				call_size - FTR_HEAP - HDR_HEAP, NULL);
	...
	else
	{
		if (!(ptr = call_mmap(call_size)))
			return (NULL);
		ft_first_free_chunk(ptr + HDR_HEAP, call_size - FTR_HEAP - HDR_HEAP,
				ft_find_prev(ptr + HDR_HEAP, *begin_free));
...
void			*ft_malloc(size_t size)
{
	...
	if (size < TINY_MAX_SIZE)
		return (ft_tiny_small(size, getpagesize() * TINY_PAGES,
					&(g_ptr.tiny_free_begin)));
	else if (size < SMALL_MAX_SIZE)
		return (ft_tiny_small(size, getpagesize() * SMALL_PAGES,
